...
import { parseSvg, optimizeSvg, cleanSymbolAttributes } from './helpers/parsing'
...
async function processSvg({
    ...
    try {
        ...
        const relativeToInclude = path.relative(
            absoluteInclude,
            absoluteFilePath
        )
        ...
        async buildStart() {
            ...
            for (const include of normalizedIncludes) {
                ...
                if (!stats?.isDirectory()) {
                    this.error(
                        `The include "${include}" is not a valid directory`
                    )
                }
            ...
            if (spriteMap.size === 0) {
                ...
                this.info(
                    `${LOG_PLUGIN_NAME} Generated ${spriteMap.size} SVG sprites`
                )
            }
        },
